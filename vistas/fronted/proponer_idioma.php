...
<div class="fade-up visible" style="max-width: 600px; margin: 0 auto;">
    <header style="text-align: center; margin-bottom: 30px;">
        <h2><?php echo $lang['frontend_propose_language_title']; ?></h2>
        <p style="color: #666;"><?php echo $lang['frontend_propose_language_desc']; ?></p>
    </header>

    <div class="about-box" style="padding: 40px; background: white; border-radius: 20px; border: 1px solid #eee;">
        <form action="../../controllers/JuegoController.php?action=sugerir_idioma_independiente" method="POST">
            <div class="form-group" style="margin-bottom: 25px; text-align: left;">
                <label style="font-weight: 800; font-size: 0.8rem; display: block; margin-bottom: 10px;"><?php echo $lang['frontend_propose_language_label_name']; ?></label>
                <input type="text" name="nombre_idioma" placeholder="<?php echo $lang['frontend_propose_language_placeholder']; ?>" required
            ...
            <button type="submit" class="btn-confirm" style="width:100%; border:none; cursor:pointer; background:var(--graphite); color:white; padding:15px; border-radius:50px; font-weight:700; text-transform: uppercase;">
                <?php echo $lang['frontend_propose_language_submit']; ?>
            </button>
        </form>
    </div>
</div>
